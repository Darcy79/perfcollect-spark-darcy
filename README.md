...
3. **运行采集器**：打开 `collector` 文件夹，在**地址栏**输入 `cmd` 回车（会直接弹出定位到该目录的命令行），然后：
   ...
   ```
   使用 `start_perfcollect.bat` 的实时看板时：微信小游戏仍需在启动向导中确认具体
   `appbrand` 进程；测普通 APK 时直接在“被测应用”下拉选择对应应用，确认应用后即可
   点击“开始采集”，工具会按包名自动解析其主进程。
4. **停止**：按 **Ctrl + C** 一次（或在看板点「⏹ 停止采集」）。
5. **结果文件**：`collector\output\<时间戳>\` 文件夹——内含 jsonl 原始数据与自动生成的自包含 HTML 报告；若发生崩溃、ANR、系统回收或进程退出，还会留下 `*.events.jsonl` / `*.crash.log` 诊断旁车；看板历史报告页也可直接打开。
...
- 实时打点：采集首点默认进入 `Label1`；需要标记场景切换时点击一次紧凑的 `＋ 分段`，系统按当前最新采样点闭合当前段并进入下一颜色。点击任意 Label 色块即可改名，编辑时当前段仍随采样继续增长
...
| `指标说明.md` | ✅ 现行 V0.8 | 指标口径权威文档：Jank 节奏校准、统计口径、数据健全性自检 |
