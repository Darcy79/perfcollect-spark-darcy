...
> **外部模型归属**：`Codex` = GitHub Codex CLI，本项目 v74 起的外部模型交付均由同一会话完成，
> 模型为 **`gpt-5.6-sol`**（2026-09 核实自本机 Codex 配置与会话记录）；`主会话` = 本机调度 / 审核 / 提交推送侧。

---

## 当前状态（2026-09-22，v105）

| 项 | 值 |
|---|---|
| 前端资源版本 | **v83**（`web/index.html` 3 处、`web/report.html` 4 处 `?v=`） |
| 测试 | Python **300** 条 + JS **182** 断言（全绿） |
...
## v105（2026-09-22 · Codex）—— 本机目标隐私隔离与诊断日志完整保留

- **运行态配置隔离**：受版本控制的 `config.json` 只保留公共默认值；向导与热切换将最近选择的
  `package/process_pattern` 原子写入 `config.local.json`，启动时透明合并，命令行参数仍为最高优先级。
- **隐私与分发保护**：`*.local.json` 已加入 Git 忽略规则，zip 打包器也按文件名模式明确排除，
  常测应用包名不会随代码提交或分发包暴露；本机文件损坏时自动降级到公共默认配置。
- **诊断日志无损**：一秒重复限流只作用于微信业务 `app_log`；Java/Native 崩溃、ANR、系统回收、
  进程日志及关联堆栈不再去重，`*.crash.log` 完整保留同秒重复行。
- **回归**：增加公共/本机配置合并、基础文件不可变、UTF-8 尾部换行、损坏降级、诊断不限流与
  console 继续限流测试；Python **295 → 300**，JS 保持 **182**，前端资源保持 **v83**。

---

## v104（2026-09-22 · Codex）—— 崩溃证据日志与进程生命周期容错

- **全应用诊断日志**：logcat 监听扩展到普通 APK 与微信小游戏，按目标 PID、包名和完整
  进程名关联 Java FATAL、Native Fatal signal、ANR 与系统低内存回收，避免串入其他应用日志。
- **保守定性**：只有明确系统日志证据才标记崩溃、ANR 或系统回收；目标进程单纯消失只记录
  “进程退出、原因待日志确认”，不把退后台、主动退出或 adb 瞬断误报成闪退。
- **可定位产物**：结构化事件继续写入 `*.events.jsonl`；存在诊断事件时额外惰性生成
  `*.crash.log`，保留时间、PID、tag、级别与完整关联堆栈，控制台同步打印显著告警。
- **FPS 精确恢复**：仅在已确认 PID 退出或切换时通知 FPS 采集线程丢弃旧 SurfaceFlinger
  图层及帧基线，下一采样重新绑定；正常加载、静态画面中的 FPS=0 不触发重置。
- **报告事件降噪**：历史看板用蓝/橙/红虚线区分重启、退出、崩溃类事件，完整堆栈只留在
  诊断文件，不在图上重复铺满标线。
- **回归**：覆盖 Java/Native 崩溃归属、ANR/系统回收、他进程隔离、生命周期去重、诊断
  文件筛选与 FPS PID 变更重绑；Python **282 → 295**，JS **181 → 182**，前端资源 **v82 → v83**。

---

## v103（2026-09-22 · Codex）—— 普通 APK 可从启动向导直接开始采集

- **修复启动前目标切换失效**：看板启动等待期注册独立目标选择回调；从“被测应用”
  下拉选择已安装 APK 后，包名与空进程模式会立即同步到本次待启动目标及配置。
- **区分两类启动策略**：普通 APK 不再强制选择微信 AppBrand 候选 PID，点击“开始采集”
  后由 `PidResolver` 按包名解析主进程；微信小游戏仍必须确认候选 PID，保留多实例防采错保护。
- **目标一致性**：用户确认后创建的 resolver、FPS/CPU/内存/网络采集器、状态与 JSONL
  全部使用最终选中的包名，不再回落到程序启动时的旧微信目标。
- **前端反馈**：普通 APK 的向导改为“确认要采集的应用”，明确提示无需选择 AppBrand
  进程并直接启用开始按钮；切回微信会恢复候选进程选择界面。
- **回归**：新增真实 HTTP 流程测试，覆盖“切换 APK → direct 向导 → 无 PID 开始请求”；
  Python **281 → 282**，JS **181**，前端资源 **v81 → v82**。
