...
| 48min 长测 | `20260918_103516`：2867 个报告点，间隔中位 1008ms、最大 1114ms，无 >1.5s 断档；PID/SurfaceView 层全程不变，无指标错误码 | 采集链路通过 30min+ 验收；后半程 FPS 回落、温度到 44°C且 PSS 上升，需固定场景复测归因 |
...
| 30min+ 长时采集 | ✅ 已验证 | 荣耀 ADT-AN00 完成 48.2 分钟 / 2867 点长测；采集连续、PID/图层稳定、HTML 成功导出。长报告悬停重绘闪烁已于 v91 修复；性能后半程回落待固定场景复测归因 |
